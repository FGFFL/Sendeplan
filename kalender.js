...
function getDateKey(date) {
  ...
  return `${year}-${month}-${day}`; // z. B. 2025-05-13
}
...
function applyWeekTemplate(name, baseDate) {
  ...
  const sourceStartDate = getMonday(firstDate);

  const targetStartDate = getMonday(baseDate);
  const targetDates = getWeekRange(targetStartDate).map(getDateKey);

  // Zuerst alte Beiträge in der Zielwoche löschen
